...
export function findDeviceByIp(state,ip){ for(const d of state.devices){ if(d.config.ipSettings?.ip===ip)return {device:d,intf:null}; for(const intf of Object.values(d.config.interfaces))if(intf.ip===ip)return {device:d,intf}; } return null; }
...
function learnAlongPath(state,path,target){ for(let i=0;i<path.length;i++){ const d=state.devices.find(x=>x.id===path[i]); if(!d)continue; if(["switch","multilayer-switch","bridge"].includes(DEVICE_TYPES[d.type].kind)){ const next=path[i+1]||target.id; const n=neighbors(state,d.id).find(x=>x.device.id===next); if(n&&!d.config.macTable.some(e=>e.mac===firstMac(target)))d.config.macTable.push({vlan:d.config.interfaces[n.local.port]?.vlan||1,mac:firstMac(target),port:n.local.port,type:"DYNAMIC"}); } const ip=target.config.ipSettings?.ip||Object.values(target.config.interfaces).find(x=>x.ip)?.ip; if(ip&&!d.config.arpTable.some(e=>e.ip===ip))d.config.arpTable.push({ip,mac:firstMac(target),type:"dynamic"}); }}
function firstMac(d){ return Object.values(d.config.interfaces)[0]?.mac||randomMac(); }
